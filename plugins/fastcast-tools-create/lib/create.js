...
module.exports = function create(entities, levels, techs, options) {
	...
	return baseConfig
		...
		.then(function (bemToolsConf) {
			var pluginConf =
				(bemToolsConf && bemToolsConf.plugins && bemToolsConf.plugins.create) || {}
		...
		.then(function (config) {
			...
			return Promise.all(
				entities.map(function (input) {
					...
					return Promise.all(
						(isFileGlob ? braceExpansion(input) : [input]).map(function (
							filepathOrInput,
						) {
								...
								currentLevels.map(function (relLevel) {
									...
									return config.level(level).then(function (levelOptions) {
										...
										var levelScheme = levelOptions.scheme,
											...
											currentTechs = uniq(
												[].concat(levelOptions.techs || [], techs),
											),
											entity
										...
										options.excludeTech &&
											(currentTechs = currentTechs.filter(function (tech) {
												return options.excludeTech.indexOf(tech) === -1
											}))

										return Promise.all(
											currentTechs.map(function (tech) {
