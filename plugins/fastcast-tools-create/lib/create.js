...
module.exports = function create(entities, levels, techs, options) {
    ...
    return baseConfig
        .module('fastcast-tools')
        .then(function(bemToolsConf) {
            var pluginConf =
                (bemToolsConf &&
                    bemToolsConf.plugins &&
                    bemToolsConf.plugins.create) ||
                {};
        ...
        .then(function(config) {
            if (!levels || !levels.length) {
                ...
                levels =
                    levelByCwd || (defaultLevels.length ? defaultLevels : cwd);
            }
            ...
            return Promise.all(
                entities.map(function(input) {
                    var isFileGlob = typeof input === 'string';

                    return Promise.all(
                        (isFileGlob ? braceExpansion(input) : [input]).map(
                            function(filepathOrInput) {
                                var currentLevels = levels;

                                if (typeof filepathOrInput === 'string') {
                                    var currentLevel = path.dirname(
                                        filepathOrInput,
                                    );
                                    currentLevel !== '.' &&
                                ...
                                return Promise.all(
                                    currentLevels.map(function(relLevel) {
                                        ...
                                        return config
                                            ...
                                            .then(function(levelOptions) {
                                                ...
                                                if (isFileGlob) {
                                                    ...
                                                    if (
                                                        filepathOrInput.modName
                                                    ) {
                                                        filepathOrInput.mod = {
                                                            name:
                                                                filepathOrInput.modName,
                                                        };
                                                        filepathOrInput.mod.val =
                                                            filepathOrInput.modVal ||
                                                            filepathOrInput.modVal !==
                                                                false;
                                                ...
                                                options.excludeTech &&
                                                    (currentTechs = currentTechs.filter(
                                                        function(tech) {
                                                            return (
                                                                options.excludeTech.indexOf(
                                                                    tech,
                                                                ) === -1
                                                            );
                                                        },
                                                ...
                                                return Promise.all(
                                                    currentTechs.map(function(
                                                        ...
                                                        var pathToFile = buildPath(
                                                                ...
                                                                levelOptions.schemeOptions ||
                                                                    levelOptions,
                                                            ),
                                                            absPathToFile = path.join(
                                                                path.resolve(
                                                                    level,
                                                                ),
                                                                pathToFile,
