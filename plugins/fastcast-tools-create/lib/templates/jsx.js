...
module.exports = function (entity, naming) {
    ...
    var valBoolean = typeof entity.mod.val === 'boolean' 
      ...
      !valBoolean ? "import React from 'reactify'" : '//without react',
			...
			'',
      `import './${entity.block}_${entity.mod.name}${!valBoolean ? '_' + entity.mod.val : ''}.css'`,
			`import { blockName } from '../${entity.block}/${entity.block}'`,
      !valBoolean ? '' : null,
      !valBoolean ? `const ${capitalizeBlock}${toCamelCase(entity.mod.name)} = ({children}) => <div className={blockName({ ${entity.mod.name}: ${modVal} })}>{children}</div>` : '//primitive application with mods and css',
      entity.mod.name
      ? `export const ${capitalizeBlock}${toCamelCase(entity.mod.name)}${!valBoolean ? toCamelCase(entity.mod.val) : ''} = withBemMod(blockName(), { ${entity.mod.name}: ${modVal} }${!valBoolean ? ',' + capitalizeBlock : ''}${!valBoolean ? toCamelCase(entity.mod.name) : ''})`
    ...
    var capitalizeElem = toCamelCase(entity.elem)
      ...
      '',
		...
		first = [
			...
			'',
			`const ${entity.block} = ({children}) => <div className={blockName()}>{children}</div>`,
			'',
			`export default ${entity.block}`,
		]
	}
